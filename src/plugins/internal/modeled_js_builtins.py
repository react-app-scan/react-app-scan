...
def react_builtin_func_useMemo(G: Graph, caller_ast, extra, _, *args):
    loggers.main_logger.info(f"react_builtin_func_useMemo, args: {args}, caller_ast: {caller_ast}, cur_scope: {G.cur_scope}, cur_component: {G.cur_jsx_component}")
...
def react_builtin_func_useState(G: Graph, caller_ast, extra, _, *args):
    ...
    loggers.main_logger.info(f"react_builtin_func_useState, args: {args}, caller_ast: {caller_ast}, cur_scope: {G.cur_scope}, cur_component: {G.cur_jsx_component}")
...
def axios_get(G: Graph, caller_ast, extra, _, *args):
    ...
    if not resolved_objs:
        ...
        if mark_tainted:
            G.set_node_attr(returned_obj, ('tainted', True))
        G.add_blank_func_as_prop(func_name='json', parent_obj=returned_obj, python_func=builtin_fetch_res_json if mark_tainted else builtin_fetch_res_json_not_tainted)
