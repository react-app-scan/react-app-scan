...
class HandleIf(Handler):
    ...
    def process(self):
        ...
        for if_elem in if_elems:
            ...
            loggers.main_logger.debug('Check condition {} result: possibility: {} deterministic: {}'.format(sty.ef.i +
...
class HandleConditional(Handler):
    def process(self):
        ...
        loggers.main_logger.debug(f'Condition: {G.get_node_attr(node_id).get("code")} deterministic: {deterministic}, possibility: {possibility}')
        if deterministic and possibility == 1:
