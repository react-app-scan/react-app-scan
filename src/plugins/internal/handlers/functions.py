...
def ES6_simurun_function(G: Graph, func_ast, branches=None, block_scope=True, caller_ast=None):
    ...
    func_obj = func_objs[0] if func_objs else '?'
    func_name = G.get_node_attr(func_obj).get('name') if func_objs else '?'
    loggers.main_logger.info(f'{sty.ef.inverse}{sty.fg.green}FUNCTION STARTS {func_ast} {func_name or "{anonymous}"} , SCOPE {G.cur_scope}, DECL OBJ {func_obj}, this OBJs {G.cur_objs}, branches {branches}{sty.rs.all}')
    returned_objs, used_objs = [], []
    # update graph register for cur_func
    G.cur_func = G.get_cur_function_decl()
...
def handle_require(G: Graph, caller_ast, extra, _, module_names):
    from src.plugins.internal.modeled_js_builtins import setup_express, setup_mongoose
        ...
        if parent_obj is None:
            ...
            if options.exported_func_timeout is not None:
                ...
                returned_result, newed_objs = call_function(G, [obj],
                    extra=extra, is_new=True, mark_fake_args=mark_fake_args)
                ...
                try:
                    with timeout(seconds=options.exported_func_timeout, error_message="exported func timeout"):
                        returned_result, newed_objs = call_function(G, [obj],
                            ...
                            extra=extra, mark_fake_args=mark_fake_args)
...
def call_function(G: Graph, func_objs, args=[], this=NodeHandleResult(), extra=None,
    caller_ast=None, is_new=False, stmt_id='Unknown', func_name=None,
    mark_fake_args=False, python_callback=None, parent_map_func_objs = None):
    '''
    ...
    '''
    loggers.main_logger.info(f'calling function {func_objs}, args: {args}')
    from src.plugins.manager_instance import internal_manager
    if G.finished:
        return NodeHandleResult(), []
    ...
    for i, func_obj in enumerate(func_objs):
        ...
        if python_func: # special Python function
            if is_new:
                ...
                logger.log(ATTENTION, f'Running Python function {python_func.__name__} {func_obj} ...')
                try:
                    logger.info(f'python func args: {_args}')
                        ...
                        if G.get_node_attr(param).get('flags:string[]') \
                            ...
                            for arg_obj_node in arg_obj_nodes:
                                ...
                                for el in param_children:
                                    r = internal_manager.dispatch_node(el, extra=ExtraInfo(parent_obj=arg_obj_node, branches=branches, scope = func_scope))
                        else:
                    ...
                    if G.get_node_attr(param).get('flags:string[]') \
                        ...
                        try:
                            ...
                            for el in param_children:
                                ...
                                object_pattern_added_obj = G.add_obj_to_scope(name=r.name, scope=func_scope, ast_node=caller_ast or param,
                                                        js_type='object' if G.check_proto_pollution
                                                        else None, value=wildcard)
                                if mark_fake_args:
                ...
                if callback_functions:
                    ...
                    if func_name.endswith('map') and this and this.obj_nodes:
                        ...
                        if 'tainted' in this_obj_attr and this_obj_attr['tainted'] == True:
                            ...
                            mark_fake_args = True
                    loggers.main_logger.info(f'{func_name} calling callback function, {callback_functions}, {_args}, func_objs: {func_objs}, this: {this}, mark_fake_args: {mark_fake_args}')
                    call_function(G, callback_functions, caller_ast=caller_ast,
                        extra=extra, stmt_id=stmt_id, mark_fake_args = mark_fake_args, parent_map_func_objs=parent_map_func_objs)
        ...
        if caller_ast is not None and func_ast is not None and \
            ...
            logger.info(sty.fg.li_magenta + sty.ef.inverse + "CALLS" + 
                sty.rs.all + " caller_cpg {} -> func_ast {} (Line {} -> Line {}) {}".format(
                    caller_cpg, func_ast,
