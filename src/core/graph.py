...
class Graph:
    ...
    def export_to_CSV(self, nodes_file_name, rels_file_name, light = False):
        ...
        with open(nodes_file_name, 'w') as fp:
            headers = ['id:ID','labels:label','type','flags:string[]','jsx:label', 'lineno:int','code','childnum:int','funcid:int','classname','namespace','endlineno:int','name','doccomment', 'export', 'tainted']
            ...
            for node in nodes:
                ...
                if light:
                    if 'export' not in row:
                        self.logger.error("Node " + str(attr) + \
                                " ID: " + node_id + 
        ...
        with open(rels_file_name, 'w') as fp:
            ...
            for edge in edges:
                ...
                if edge_from not in exported_nodes_set or \
                        edge_to not in exported_nodes_set:
                        continue
        ...
        self.logger.info(f"Finished Exporting to {nodes_file_name} and {rels_file_name} in {end_time - start_time} seconds")
    ...
    def find_nearest_upper_CPG_node(self, node_id):
        ...
        while True:
            ...
            parent_node = parent_edges[0][0]
            parent_node_attr = self.get_node_attr(parent_node)
            if parent_node_attr.get('type') in ["AST_STMT_LIST", "AST_JSXChildren", "AST_JSXAttributes", "AST_JSXAttribute"]:
                result = node_id
                break
            node_id = parent_node
    ...
    def add_jsx_component_def_edge(self, comp_node, def_node, attr={}):
        self.logger.log(ATTENTION, f'Added JSX COMPONENT_DEF Edge From {comp_node} to {def_node}, type: {JSX_Edges.JSX_COMPONENT_DEF}')
    ...
    def assign_obj_nodes_to_name_node(self, name_node, obj_nodes, multi=False,
        ...
        self.logger.debug(f'Assigning Obj nodes {obj_nodes} to Name nodes {name_node}, ' + \
    ...
    def get_ast_by_scope(self, scope_node):
        ast_node = self.get_out_edges(scope_node,
                        edge_type='SCOPE_TO_AST')[0][1]
        return ast_node
    ...
    def get_node_file_content(self, node_id, file_path = None):
        ...
        """
        file_name = file_path or self.get_node_file_path(node_id)
